...
nextSize.addEventListener("click", (event) => {
    ...
    if (userSize === estimatedSize) {
      finalSize.textContent = findYourFitData.sizeCorrect.replace('%s', estimatedSize); // Localization 
    } else {
      // If user size and estimated size do not match, select an average size
      finalSize.textContent = findYourFitData.sizeRecommend.replace('%s', userSize).replace('%s', estimatedSize); // Localization 
    }
});
